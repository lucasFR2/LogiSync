...
@section('title', 'Conferência de Expedição')
@section('page-title', 'Conferência de Expedição')
@section('page-subtitle', $invoice->number . ' • ' . $invoice->recipient_name)
...
<div class="anim-entrance" style="display:flex; flex-direction:column; gap:2rem;">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:1rem;">
        ...
        <div style="display:flex; align-items:center; gap:1rem;">
            <div id="progress-counter" style="font-family:'Outfit'; font-size:1.1rem; font-weight:700; color:var(--text-secondary);">
                <span id="items-checked-count">0</span> / <span id="items-total-count">{{ $invoice->items->count() }}</span> itens conferidos
            </div>
    ...
    <div class="grid" style="grid-template-columns: 1fr 1fr 1fr; gap:1.5rem;">
        <div class="card" style="border-left:4px solid var(--accent);">
            <div class="card-body" style="padding:1.25rem;">
                <div style="font-size:0.7rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.06em; margin-bottom:0.35rem;">Nota Fiscal</div>
                <div style="font-weight:700; color:var(--text-primary); font-size:1.05rem;">{{ $invoice->number }}</div>
                <div style="font-size:0.8rem; color:var(--text-secondary); margin-top:0.15rem;">Série {{ $invoice->series }} • {{ $invoice->statusLabel() }}</div>
            </div>
        </div>
        <div class="card" style="border-left:4px solid var(--blue);">
            <div class="card-body" style="padding:1.25rem;">
                <div style="font-size:0.7rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.06em; margin-bottom:0.35rem;">Destinatário</div>
                <div style="font-weight:700; color:var(--text-primary); font-size:1.05rem;">{{ $invoice->recipient_name }}</div>
                <div style="font-size:0.8rem; color:var(--text-secondary); margin-top:0.15rem;">{{ $invoice->recipient_document ?: 'Sem documento' }}</div>
            </div>
        </div>
        <div class="card" style="border-left:4px solid var(--green);">
            <div class="card-body" style="padding:1.25rem;">
                ...
                <div style="font-weight:800; color:var(--accent); font-size:1.5rem; font-family:'Outfit';">R$ {{ number_format($invoice->total, 2, ',', '.') }}</div>
                <div style="font-size:0.8rem; color:var(--text-secondary);">{{ $invoice->items->count() }} itens • {{ $invoice->payment_method ? $invoice->paymentLabel() : 'Pagamento N/A' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="card" id="scanner-card" style="border:2px solid var(--accent); position:relative; overflow:hidden;">
        ...
        <div class="card-body" style="padding:1.5rem; display:flex; align-items:center; gap:1.5rem;">
            ...
            <div style="flex:1;">
                <label for="barcode-input" style="font-size:0.75rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.06em; margin-bottom:0.35rem; display:block;">Bipar Código de Barras (EAN) - Expedição</label>
    ...
    {{-- Items Table --}}
    <form id="conference-form" action="{{ route('invoices.confer-save', $invoice) }}" method="POST">
        @csrf
        <div class="card" style="padding:0; overflow:hidden;">
            <div class="card-header" style="background:var(--bg-hover);">
                <div style="display:flex; align-items:center; gap:0.75rem;">
                    ...
                    <h3 style="margin:0; font-family:'Outfit';">Itens para Conferência de Expedição</h3>
                </div>
                <div style="display:flex; gap:0.75rem; align-items:center;">
                    <div class="badge" style="background:var(--bg-hover); color:var(--text-primary); font-weight:700;">{{ $invoice->items->count() }} ITENS</div>
                ...
                <table style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:right; padding:1rem;">Qtd. Pedido</th>
