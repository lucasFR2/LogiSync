    ...
    <form action="{{ route('roles.update', $role) }}" method="POST">
        ...
        <div style="display:flex; flex-direction:column; gap:2rem;">
            ...
            <div class="card">
                ...
                <div class="card-body" style="padding: 2rem;">
                    @if($role->name === 'Administrador')
                        <div class="alert" style="font-size:0.9rem; font-weight:700; background: var(--blue-bg); border-color: var(--blue); color: var(--blue); margin-bottom: 1.5rem;">
                                    ...
                                    @foreach($groupPermissions as $p)
                                        <label style="display:flex; align-items:flex-start; gap:0.75rem; cursor:pointer; font-size:0.875rem; color:var(--text-secondary); transition: all 0.2s;" class="permission-item">
                                            <input type="checkbox" name="permissions[]" value="{{ $p->id }}" 
                                                ...
                                                {{ $role->name === 'Administrador' || in_array($p->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}
